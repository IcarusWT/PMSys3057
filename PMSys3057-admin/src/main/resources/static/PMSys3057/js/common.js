...
var refreshItem = function(){
	...
	var target = $('.PMSys3057_iframe[data-id="' + currentId + '"]', topWindow);
...
function createMenuItem(dataUrl, menuName) {
    ...
    $('.menuTab', topWindow).each(function() {
        if ($(this).data('id') == dataUrl) {
            if (!$(this).hasClass('active')) {
                ...
                // 显示tab对应的内容区
                $('.mainContent .PMSys3057_iframe', topWindow).each(function() {
                    if ($(this).data('id') == dataUrl) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
        $('.mainContent', topWindow).find('iframe.PMSys3057_iframe').hide().parents('.mainContent').append(str1);
