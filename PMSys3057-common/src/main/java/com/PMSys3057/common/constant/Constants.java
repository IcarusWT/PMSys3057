package com.PMSys3057.common.constant;

// 通用常量信息
public class Constants
{
    public static final String UTF8 = "UTF-8";              // UTF-8 字符集
    public static final String SUCCESS = "0";               // 通用成功标识
    public static final String FAIL = "1";                  // 通用失败标识
    public static final String LOGIN_SUCCESS = "Success";   // 登录成功
    public static final String LOGOUT = "Logout";           // 注销
    public static final String LOGIN_FAIL = "Error";        // 登录失败
    public static String AUTO_REOMVE_PRE = "true";          // 自动去除表前缀
    public static String PAGE_NUM = "pageNum";              // 当前记录起始索引
    public static String PAGE_SIZE = "pageSize";            // 每页显示记录数
    public static String ORDER_BY_COLUMN = "orderByColumn"; // 排序列
    public static String IS_ASC = "isAsc";                  // 排序的方向 "desc" 或者 "asc".
}
